...
When results are too broad, narrow in this order: read the match count first, then scope with `path`/`glob`, then tighten `pattern`, and only add `context` once the set is small. Output is capped on whole lines at 50KB/2000 lines; when truncated, narrow the search and rerun to continue.

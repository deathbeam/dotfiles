 ...
 */
function buildAnchorsBlock(resultLines: string[], anchorRange: { start: number; end: number } | null): string {
    if (!anchorRange) {
    ...
    return Buffer.byteLength(block, "utf8") <= CHANGED_ANCHOR_TEXT_BUDGET_BYTES ? block : ANCHORS_OMITTED_TEXT;
}
...
export function buildChangedResponse(input: SuccessResponseInput): ToolResult {
    ...
    const text = [anchorsBlock, warningsBlock.trimStart()].filter((section) => section.length > 0).join("\n\n");

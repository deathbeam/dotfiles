...
import { BARE_PREFIX_RE, type Anchor, type HashlineEdit } from "./parse";
...
function formatMismatchError(mismatches: HashMismatch[], fileLines: string[]): string {
    ...
    if (hintedMismatches.length > 0) {
        ...
        type AnchorCandidates = { kind: "list"; lines: number[] } | { kind: "overflow"; count: number };
        ...
        const hasAnyCandidates = perAnchor.some(
            ({ result }) => result.kind === "overflow" || (result.kind === "list" && result.lines.length > 0),
        );
        if (hasAnyCandidates) {
            out.push("");
            out.push("Did you mean (content-matched candidates for stale anchors):");
            for (const { mismatch, result } of perAnchor) {
                if (result.kind === "overflow") {
                    ...
                    for (const lineNum of result.lines) {
                        ...
                        out.push(
                            `  ${lineNum}#${freshHash}:${lineContent}   ← for stale ${mismatch.line}#${mismatch.expected}`,
                        );
                    }
                }
            }
 ...
 */
function warnBareHashPrefixLines(edits: HashlineEdit[], fileLines: string[], warnings: string[]): void {
    ...
    const fileHashSet = new Set(fileLines.map((_line, i) => computeLineHash(fileLines, i)));
    const matchCount = suspects.filter(({ hash }) => fileHashSet.has(hash)).length;

    if (matchCount > 0 || suspects.length >= 2) {
        const matchHint = matchCount > 0 ? ` ${matchCount} prefix(es) match existing line hashes in this file.` : "";
        warnings.push(
...
function buildLineIndex(content: string): LineIndex {
    ...
    return {
        ...
        lineStarts,
        hasTerminalNewline,
        visibleLineCount: hasTerminalNewline ? fileLines.length - 1 : fileLines.length,
    };
}
...
function computeInsertionBoundary(
    ...
    if (lineIndex.hasTerminalNewline && edit.pos.line === lineIndex.fileLines.length) {
        return lineIndex.visibleLineCount;
    }
...
function warnDuplicateInsert(
    ...
    const allMatch = insertLines.every((line, i) => line.trim() === adjacentLines[i]!.trim());
    if (!allMatch) return;
 ...
 */
function assembleEditResult(content: string, spans: ResolvedEditSpan[], signal: AbortSignal | undefined): string {
    let result = content;
...
export function applyHashlineEdits(
    ...
    const { mismatches } = validateAnchorEdits(edits, lineIndex, warnings, signal);
    ...
    const orderedSpans = resolveEditSpans(edits, content, lineIndex, noopEdits, signal);
